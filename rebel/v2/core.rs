...
type NativeFn =
    for<'a> fn(stack: &[Word], ctx: &'a mut EvalContext<'a>) -> Result<[Word; 2], RebelError>;
...
type SimpleLayout<'a> = Memory<&'a mut [Word], &'a mut [Word]>;

pub fn init_memory(memory: &mut [Word], symbols_size: usize) -> Result<SimpleLayout, RebelError> {
    let (symbols, heap) = memory
...
pub struct EvalContext<'a> {
    heap: &'a mut Block<&'a mut [Word]>,
    parse: Stack<Box<[Word]>>,
    ops: Stack<Box<[Word]>>,
    stack: Stack<Box<[Word]>>,
    natives: &'a [FuncDescriptor],
}

impl<'a> EvalContext<'a> {
    pub fn new(heap: &'a mut Block<&'a mut [Word]>, natives: &'a [FuncDescriptor]) -> Self {
        Self {
            heap,
            natives,
            parse: Stack(Box::new([0; 256])),
            ops: Stack(Box::new([0; 256])),
            stack: Stack(Box::new([0; 256])),
        }
    }

    pub fn pop_parse(&mut self) -> Result<&[Word], RebelError> {
        self.parse.pop_all(0)
    }

    pub fn pop_stack(&mut self) -> Result<ChunksExact<'_, Word>, RebelError> {
        self.stack.pop_all(0).map(|data| data.chunks_exact(2))
    }

    pub fn eval_parsed(&'a mut self, data: &[Word]) -> Result<(), RebelError> {
        self.stack.init()?;
        ...
        for chunk in data.chunks_exact(2) {
            ...
            let mut sp = self.stack.push_offset(value)?;
            ...
            while let Some([bp, arity]) = cur {
                if sp == bp + arity {
                    let frame = self.stack.pop_all(bp)?;
                    let op: [Word; 2] =
                    ...
                    match op {
                        [TAG_SET_WORD, sym] => {
                            ...
                            sp = self.stack.push_offset(value)?;
                        }
                        [TAG_NATIVE_FN, func] => {
                            let native_fn = {
                                self.natives
                            ...
                            // let mut new_ctx = EvalContext::new(self.heap, self.natives);
                            let result = (native_fn.func)(stack, self)?;
                            sp = self.stack.push_offset(result)?;
                        }
                        _ => {
...
// impl<'a, H, Y> ExectionContext for EvalContext<'a, H, Y>
// where
//     H: AsMut<[Word]>,
// {
//     fn do_eval(&mut self, block: Offset) -> Result<[Word; 2], RebelError> {
//         self.eval_parsed()?;
//         Ok([0, 0])
//     }
// }

impl Collector for EvalContext<'_> {
    fn string(&mut self, string: &str) -> Result<(), RebelError> {
...
// pub fn test(ctx: &mut EvalContext<&mut [Word], &mut [Word]>) -> Result<(), RebelError> {
//     ctx.eval_parsed()
// }
...
mod tests {
    ...
    fn test_eval_1() -> Result<(), RebelError> {
        ...
        let mut mem = init_memory(&mut buf, 1024)?;
